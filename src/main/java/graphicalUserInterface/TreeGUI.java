...
public class TreeGUI extends Group {
    double xDimension;
    double yDimension;
    double dataElementHeight = 50; // HAS TO BE DYNAMIC NOT STATIC EDIT
    double xSpacing = 40; //HAS TO BE DYNAMIC NOT STATIC EDIT
    double ySpacing = 70; //HAS TO BE DYNAMIC NOT STATIC EDIT
    ...
    public TreeGUI (Node root){
        ...
        if(parent.hasChildren()){
            for(Node child:parent.getChildren()){
                ...
                childTree.setLayoutY(dataElementHeight + ySpacing); //set yDimension
                lineList.add(new Line(0,0,(xDimension+(childTree.xDimension/2.0)),(dataElementHeight + ySpacing)));
                super.getChildren().add(lineList.get(lineList.size()-1));
                yDimension = childTree.getyDimension(); //WORKS BUT CORRECT THIS VALUE IS SET MANY TIMES
        ...
        for(Line line:lineList){
            ...
            line.setStartY(dataElementHeight);
            xPosition += dataElementHeight; //MISSING SPACING BETWEEN DATA ELEMENTS
        }
    ...
    public double getDataElementHeight() {
        return dataElementHeight;
    }

    public void setDataElementHeight(double dataElementHeight) {
        this.dataElementHeight = dataElementHeight;
    }
